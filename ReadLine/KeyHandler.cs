...
namespace Internal.ReadLine
{
    internal class KeyHandler
    {
        ...
        private bool IsEndOfBufferVertical() => Console2.CursorTop == Console2.BufferHeight - 1;
        ...
        {
            if (IsEndOfBufferVertical())
                Console2.SetBufferSize(Console2.BufferWidth, Console2.BufferHeight+1);

            Console2.SetCursorPosition(0, Console2.CursorTop+1);
        }

        private void MoveCursorUpEnd()
        {
            Console2.SetCursorPosition(0, Console2.CursorTop - 1);
            MoveCursorEnd();
        }
        ...
        public KeyHandler(IConsole console, List<string> history, IAutoCompleteHandler autoCompleteHandler)
        {
            ...
            _keyActions["Tab"] = () =>
            {
                ...
                else
                {
                    ...
                    _completionStart = text.LastIndexOf(wordToComplete)-1;
                    //_completionStart = text.LastIndexOfAny(autoCompleteHandler.Separators);
                    // this resets back the position of completion to the completion character
                    // however for "mid-word" completions, we would want to reset back to the beginning of that
